import { useState, useEffect } from "react";
  ...
  export default function Home() {
    ...
    const { carts, addToCart: cartAddToCart, canAddItem, clearCart, getTotalItems, setUserLocation, getAllCartsWithDelivery, updateChefStatus, fetchChefStatuses } = useCart();
    ...
    useEffect(() => {
      ...
      // For this specific change, the intention is to remove the *duplicate* WebSocket logic and rely on `useCustomerNotifications`.
      // If `useCustomerNotifications` itself handles showing toasts for status updates, then this block is fine.
      // If not, we might need to re-evaluate how toasts are triggered based on `chefStatuses`.
                      ...
                      filteredProducts.map(product => {
                        ...
                        return (
                          <ProductCard
                            ...
                            isAvailable={realtimeAvailability?.isAvailable ?? product.isAvailable}
                            stock={realtimeAvailability?.stock ?? product.stockQuantity}
                          />
                        );
                      })
                    )}
